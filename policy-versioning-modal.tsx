...
import { CompanyAcceptanceForm } from "./components/company-acceptance-form"
import { DeadlineIndicator } from "./components/deadline-indicator"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"

interface PolicyVersioningModalProps {
  isOpen: boolean
  onAccept: (version: string, acceptanceType?: "individual" | "company", companyInfo?: any) => void
  onDecline: () => void
  policyData: PolicyData
  userCanAcceptForCompany?: boolean
}

export default function PolicyVersioningModal({
  isOpen,
  onAccept,
  onDecline,
  policyData,
  userCanAcceptForCompany = false,
}: PolicyVersioningModalProps) {
  ...
  const [acceptanceType, setAcceptanceType] = useState<"individual" | "company">("individual")
  ...
  const handleAccept = () => {
    onAccept(currentVersion.version, acceptanceType, acceptanceType === "company" ? companyInfo : undefined)
  }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onDecline()}>
      <DialogContent className="max-w-5xl max-h-[90vh] p-0">
        {/* Custom Close Button */}
        <Button variant="ghost" size="sm" className="absolute right-4 top-4 z-10 h-8 w-8 p-0" onClick={onDecline}>
          <X className="h-4 w-4" />
        </Button>
